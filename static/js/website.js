...
const websiteEditor = {
    ...
    setupDynamicForms: function() {
        ...
        if (addServiceBtn) {
            addServiceBtn.addEventListener('click', function() {
                ...
                const removeButton = newServiceItem.querySelector('.remove-service-btn');
            ...
            addFeatureBtn.addEventListener('click', function() {
                ...
                const featureTemplate = `
                    <div class="card mb-3 feature-item">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <h6 class="mb-0">Feature ${featureCount + 1}</h6>
                ...
                const removeButton = newFeatureItem.querySelector('.remove-feature-btn');
                removeButton.addEventListener('click', function() {
        ...
        if (addTestimonialBtn) {
            addTestimonialBtn.addEventListener('click', function() {
                ...
                const testimonialCount = testimonialsContainer.querySelectorAll('.testimonial-item').length;
                
                const testimonialTemplate = `
                    <div class="card mb-3 testimonial-item">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <h6 class="mb-0">Testimonial ${testimonialCount + 1}</h6>
                ...
                testimonialsContainer.insertAdjacentHTML('beforeend', testimonialTemplate);
                
                // Setup remove button for the new testimonial
                const newTestimonialItem = testimonialsContainer.lastElementChild;
                const removeButton = newTestimonialItem.querySelector('.remove-testimonial-btn');
                removeButton.addEventListener('click', function() {
    ...
    updateColorPreview: function() {
        ...
        const accentColor = document.getElementById('accentColor')?.value || '#fd7e14';
    ...
    collectContentFormData: function() {
        ...
        if (servicesContainer) {
            ...
            serviceItems.forEach((item, index) => {
                const service = {
                    title: item.querySelector(`[name="services[${index}].title"]`)?.value || '',
                    description: item.querySelector(`[name="services[${index}].description"]`)?.value || '',
                    icon: item.querySelector(`[name="services[${index}].icon"]`)?.value || ''
                };
        ...
        if (testimonialsContainer) {
            ...
            testimonialItems.forEach((item, index) => {
                const testimonial = {
                    quote: item.querySelector(`[name="testimonials[${index}].quote"]`)?.value || '',
                    author: item.querySelector(`[name="testimonials[${index}].author"]`)?.value || '',
                    position: item.querySelector(`[name="testimonials[${index}].position"]`)?.value || ''
                };
    ...
    collectSettingsFormData: function() {
        const settingsFormData = {
            ...
            content: {
                enableContactForm: document.getElementById('enableContactForm')?.checked || false
            }
        };
    ...
    saveWebsiteContent: function(websiteId, formData) {
        ...
        document.getElementById('loadingMessage').textContent = 'Saving website content...';
    ...
    saveWebsiteStyle: function(websiteId, formData) {
        ...
        // Update loading message
        document.getElementById('loadingMessage').textContent = 'Saving website style...';
    ...
    saveWebsiteSettings: function(websiteId, formData) {
        ...
        // Update loading message
        document.getElementById('loadingMessage').textContent = 'Saving website settings...';
    ...
    deleteWebsite: function(websiteId) {
        // Hide delete modal
        const deleteModal = bootstrap.Modal.getInstance(document.getElementById('deleteWebsiteModal'));
        if (deleteModal) {
            deleteModal.hide();
        }
        ...
        // Update loading message
        document.getElementById('loadingMessage').textContent = 'Deleting website...';
    ...
    setupRegenerateContent: function() {
        ...
        regenerateContentBtn.addEventListener('click', function() {
            ...
            const formData = {
                ...
                additional_info: document.getElementById('regenerateAdditionalInfo').value
            };
        ...
        if (regenerateColorsBtn) {
            regenerateColorsBtn.addEventListener('click', function() {
                const businessType = document.getElementById('regenerateBusinessType')?.value || 'business';
                const industry = document.getElementById('regenerateIndustry')?.value || 'general';
    ...
    regenerateContent: function(websiteId, formData) {
        ...
        // Update loading message
        document.getElementById('loadingModalLabel').textContent = 'Regenerating Content';
        document.getElementById('loadingMessage').textContent = 'Our AI is creating new content for your website. This may take a minute...';
    ...
    regenerateColorScheme: function(businessType, industry) {
        ...
        // Update loading message
        document.getElementById('loadingModalLabel').textContent = 'Generating Colors';
        document.getElementById('loadingMessage').textContent = 'Generating a new color scheme for your website...';
        ...
        fetch(`/api/colors/generate?business_type=${businessType}&industry=${industry}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${authManager.getToken()}`
            }
        ...
        .then(data => {
            ...
            if (data && data.colors) {
                ...
                document.getElementById('primaryColorText').value = data.colors.primary;
                
                document.getElementById('secondaryColor').value = data.colors.secondary;
                document.getElementById('secondaryColorText').value = data.colors.secondary;
                ...
                document.getElementById('backgroundColorText').value = data.colors.background;
...
const websiteList = {
    ...
    sortWebsites: function() {
        ...
        websiteItems.sort((a, b) => {
            ...
            const dateA = new Date(a.querySelector('.created-date')?.textContent || 0);
            const dateB = new Date(b.querySelector('.created-date')?.textContent || 0);
    ...
    toggleWebsiteStatus: function(websiteId, status) {
        ...
        .then(response => {
            if (!response.ok) {
                throw new Error(`Failed to ${status === 'published' ? 'publish' : 'unpublish'} website`);
            }
            return response.json();
        })
        .then(data => {
            // Show success message
            showNotification(`Website ${status === 'published' ? 'published' : 'unpublished'} successfully`, 'success');
        ...
        document.querySelectorAll('.delete-website-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (websiteId) {
                    ...
                    document.getElementById('deleteWebsiteTitle').textContent = websiteTitle;
                }
            });
        });
